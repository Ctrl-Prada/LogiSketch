...
import { WarehouseCanvas, CanvasHandle } from './components/WarehouseCanvas';
...
export default function App() {
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-xl font-bold text-white">Dimensões do Galpão</h3>
      </div>
      <div className="flex items-center gap-2">
          <div className="flex-1">
             <DarkInput 
                label="MAIOR DIMENSÃO (Eixo X)"
                value={data.width || ''}
                onChange={(e: any) => setData({ ...data, width: Number(e.target.value) })}
                placeholder="Ex: 100"
            />
          </div>
          <button onClick={handleSwapStep1} className="mt-6 p-2 text-gray-400 hover:text-silicon-orange transition-colors">
              <RotateCw className="w-5 h-5" />
          </button>
          <div className="flex-1">
            <DarkInput 
                label="MENOR DIMENSÃO (Eixo Y)"
                value={data.length || ''}
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-xl font-bold text-white">Layout de Armazenagem</h3>
      </div>
      ...
      <div className={`p-4 rounded-xl border shadow-lg ${inputMode === 'MEZZANINE' ? 'bg-silicon-input border-silicon-teal/30' : 'bg-silicon-input border-gray-700'}`}>
          <div className={`text-xs font-bold uppercase mb-3 tracking-widest ${inputMode === 'MEZZANINE' ? 'text-silicon-teal' : 'text-silicon-orange'}`}>
             {inputMode === 'MEZZANINE' ? 'Adicionar Mezanino' : 'Adicionar Bloco'}
          </div>
              ...
              {data.storage.racks.map((rack, idx) => (
                  <li key={rack.id} className={`text-xs bg-silicon-input p-3 rounded-lg border flex justify-between items-center group transition-colors ${rack.type === 'MEZZANINE' ? 'border-silicon-teal/30 hover:border-silicon-teal/50' : 'border-gray-700 hover:border-silicon-orange/50'}`}>
                      <div className="flex flex-col">
                        <span className={`font-bold text-sm ${rack.type === 'MEZZANINE' ? 'text-silicon-teal' : 'text-white'}`}>{rack.label}</span>
                        <span className="text-gray-500">Pos: {rack.x.toFixed(2)}m, {rack.y.toFixed(2)}m</span>
                      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden bg-silicon-dark font-sans">
      ...
      {/* Left Panel: Controls */}
      <div className="w-full lg:w-[400px] bg-silicon-surface border-r border-gray-800 flex flex-col h-screen shadow-2xl z-20">
        <div className="p-8 border-b border-gray-800 bg-silicon-gradient relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <p className="text-white/80 text-xs mt-1 font-medium tracking-widest uppercase">Silicon Group Pro Tool</p>
            </div>
        </div>
      ...
      >
        ...
        <div className="absolute bottom-8 right-8 flex flex-col gap-2">
            <div className="bg-silicon-surface/90 backdrop-blur border border-gray-800 p-3 rounded-lg shadow-2xl text-xs text-gray-400">
                <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-silicon-orange"></span>
                    <span className="font-bold text-white">Silicon Group</span> Visualizer
                </div>
